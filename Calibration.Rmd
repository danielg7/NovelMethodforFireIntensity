...
#justOne_thermo$Seconds <- as.numeric(seconds(justOne_thermo$Time - min(justOne_thermo$Time)))
#justOne_both$Seconds <- as.numeric(seconds(justOne_both$Time - min(justOne_both$Time)))

#justOne_both <- merge(justOne,justOne_full,by="Time",all = TRUE)
#justOne_both <- merge(justOne_both,justOne_thermo,by="Time",all.x=TRUE)
...
plotPicture <- readPNG("example.png")
...
# 
# 
# names(intermediate)[3] <- "Partial_iButton"
# names(intermediate)[5] <- "Full_iButton"
# names(intermediate)[22] <- "Thermocouple"
# 
# final <- melt(intermediate, id.vars = "Time", measure.vars = c("Partial_iButton","Full_iButton","Thermocouple"))
# 
# final$Seconds <- as.numeric(seconds(final$Time - min(final$Time)))
# 
# names(final) <- c("Time","Type","Anomaly","Seconds")
# levels(final$Type) <- c("Partial iButton","Complete iButton","Thermocouple")
...
fig1_plotC <- ggplot(data = final, aes(x = as.numeric(Seconds), y= log(Temperature), colour = Type))+
  ...
  #          aes(x = as.numeric(Seconds), y = log(Temperature)))+
  #geom_line(data = justOne_thermo, colour="blue", size = 1,
   #         aes(x = as.numeric(Seconds), y = log(Temperature)))+
  #geom_line(colour = "red", size = 1,
   #         aes(x = as.numeric(Seconds), y = log(Temperature_sub)))


grid.arrange(fig1_plotA,fig1_plotB,fig1_plotC,nrow=2,ncol=2)
```

```{r Figure2_FuelLoad, echo = FALSE, warning=FALSE}

fig2_plotA<- ggplot(data = reMergeAnalysis)+
  ggtitle("A.")+
  myTheme+
     theme(plot.title = element_text(size=12,
                                  hjust="0",
         ...
         legend.position=c(.8,.9),legend.direction="vertical")+
  xlab("Fuel Load (g)")+
  ylab("Maximum Recorded Temperature Anomaly (C)")+
  geom_point(aes(x = FuelLoad_g, y =  maxTemp_subset_iButton))+
  geom_smooth(method="lm",aes(x = FuelLoad_g, y =  maxTemp_subset_iButton),se=FALSE)

fig2_plotB <- ggplot(data = reMergeAnalysis)+
  ggtitle("B.")+
  myTheme+
     theme(plot.title = element_text(size=12,
                                  hjust="0",
         ...
         legend.position=c(.8,.9),legend.direction="vertical")+
  xlab("Fuel Load (g)")+
  ylab("Summed Temperature Anomaly up to the Peak (C)")+
  geom_point(aes(x = FuelLoad_g, y = sumTemp_subset_iButton))+
  geom_smooth(method="lm",aes(x = FuelLoad_g, y = sumTemp_subset_iButton),se=FALSE)



grid.arrange(nrow=1,fig2_plotA,fig2_plotB)
```

```{r Figure3_CompareFuelLoad, echo=FALSE, warning=FALSE}
...
fig3_plotC <- ggplot(data = reMergeAnalysis)+
  ...
  ylab(intensity_lab)+
  xlab(tmax_lab)+
  geom_point(aes(x = maxTemp_t, y = Intensity))+
  geom_smooth(method="lm",aes(x = maxTemp_t, y = Intensity),se=FALSE)
...
fig3_plotF <- ggplot(data = reMergeAnalysis)+
  ...
  ylab(intensity_lab)+
  xlab(imax_lab)+
  geom_point(aes(x = maxTemp_i, y = Intensity))+
  geom_smooth(method="lm",aes(x = maxTemp_i, y = Intensity),se=FALSE)
...
```{r Figure4_Thermocouples, echo=FALSE, warning=FALSE}
imax_lab <- expression(paste(I[max], " (", degree, "C)", sep=""))
tmax_lab <- expression(paste(T[max], " (", degree, "C)", sep=""))
isum_lab <- expression(paste(I[sum], " (", degree, "C)", sep=""))
tsum_lab <- expression(paste(T[sum], " (", degree, "C)", sep=""))

fig4_plotA<- ggplot(data = reMergeAnalysis)+
  ggtitle("A.")+
  myTheme+
     theme(plot.title = element_text(size=12,
                                  hjust="0",
  ...
  ylab(tmax_lab)+
  xlab(imax_lab)+
  geom_point(aes(x = maxTemp_subset_iButton, y =  maxTemp_t))+
  geom_smooth(method="lm",aes(x = maxTemp_subset_iButton, y =  maxTemp_t),se=FALSE)

fig4_plotB <- ggplot(data = reMergeAnalysis)+
  ggtitle("B.")+
  myTheme+
     theme(plot.title = element_text(size=12,
                                  hjust="0",
  ...
  ylab(tsum_lab)+
  geom_point(aes(x = sumTemp_subset_iButton, y = sumTemp_t))+
  geom_smooth(method="lm",aes(x = sumTemp_subset_iButton, y = sumTemp_t),se=FALSE)


fig4_plotC <- ggplot(data = reMergeAnalysis)+
  ggtitle("C.")+
  myTheme+
     theme(plot.title = element_text(size=12,
                                  hjust="0",
  ...
  ylab(tmax_lab)+
  geom_point(aes(x = sumTemp_subset_iButton, y = maxTemp_t))+
  geom_smooth(method="lm",aes(x = sumTemp_subset_iButton, y = maxTemp_t),se=FALSE)


grid.arrange(nrow=1,fig4_plotA,fig4_plotB, fig4_plotC)
...
### Check residuals for effects of wx
...
basicPlotTC <- ggplot(data = testResidsDF_i)+
  myTheme+
...
```{r}
summary(lm(iButton_resids ~ RH_mean, testResidsDF_tc))
```
...
```{r}
summary(lm(thermocouple_resids ~ WS_mean, iButton_resids))
```
